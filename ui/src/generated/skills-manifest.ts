...
const _manifestData = {
  "skills": [
    ...
    },
    {
      ...
      "iconType": "svg",
      "description": "将用户提供的主题、素材、草稿、 Markdown 或正文先整理为可审阅、可复用的公众号发布源稿，稳定生成 title、summary、cover_prompt、markdown 四段结果；确认后按内置主题规范生成微信兼容内联样式 HTML。默认中间结果仅保留在对话、执行内存或内存直供 stdin 中，不生成过程文件；用户明确要求时，可通过确定性渲染链路落盘 final.html，或 source.md + final.html，并在 HTML 主交付后追加可选微信封面图与 cover.png。",
      "homepage": "https://github.com/yumih1129/skills-repo/tree/main/skills/wechat-article-generator",
      "tags": [
        "微信公众号",
        "发布源稿",
        "HTML"
